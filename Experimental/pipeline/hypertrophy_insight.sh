...
while getopts f:i:j:t:u:o:m:s: flag
do
    case "${flag}" in
        f) folder=${OPTARG};;
        i) img1=${OPTARG};;
...
python3 segment.py -folder "$folder" -image "$img1" -outfolder "$outfolder" -model "$model" -show "$show"
python3 segment.py -folder "$folder" -image "$img2" -outfolder "$outfolder" -model "$model" -show "$show"
...
python3 deform.py -img1 "$folder$img1" -mask1 "$mask1" -img2 "$folder$img2" -mask2 "$mask2" -out "$outfolder" -thermal1 "$folder$thermal1" -thermal2 "$folder$thermal2" -show "$show"
...
python3 difference.py -thermal1 "${outfolder}thermal_deformed_${thermal1}" -thermal2 "${outfolder}thermal2.png" -out "$outfolder" -show "$show"